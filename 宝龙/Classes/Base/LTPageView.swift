...
private let glt_sliderDefaultWidth: CGFloat = 290 * 0.5 //40.010101010
public class LTLayout: NSObject {
    ...
    @objc public var sliderHeight: CGFloat =  44.0 //44.0
    ...
    /* 放大标题的倍率 */
    @objc public var scale: CGFloat = 1.0   //1.2
...
public class LTPageView: UIView {
    ...
    public lazy var pageTitleView: UIView = {
        let pageTitleView = UIView(frame: CGRect(x: 0, y: 76 * 0.5 , width: self.bounds.width, height: 95 * 0.5))
    ...
    private lazy var scrollView: UIScrollView = {
        ...
        layout.isSinglePageView ? viewControllerY = 0.0 : (viewControllerY = 0.0)
    ...
    @objc public init(frame: CGRect, currentViewController: UIViewController, viewControllers:[UIViewController], titles: [String],titleS:[String], layout: LTLayout) {
...
extension LTPageView {
    ...
    private func buttonsLayout() {
        ...
        let subH = pageTitleView.bounds.height
        let subY: CGFloat = 0
        ...
        for index in 0..<titles.count {
            ...
            let button = subButton(frame: CGRect(x: upX, y: subY, width: subW, height: subH), flag: index, title: titles[index],titleS: titlesS[index], parentView: sliderScrollView)
            //button.setTitleColor(layout.titleColor, for: .normal)
              //  button.isSelected = false
           // button.setImage(UIImage.init(named: titles[index]), for: .normal)
            if index == 0 {
               // button.setTitleColor(layout.titleSelectColor, for: .normal)
               // button.setImage(UIImage.init(named: titlesS[index]), for: .normal)
...
//        }
        ...
        let index = Int(offsetX / scrollW)
        let nextIndex = Int(index)
        ...
        if offsetX > glt_startOffsetX { // 向左滑动
           ...
           // nextIndex = nextIndex + 1
            let index = Double(offsetX / scrollW)
            if (index == 0 || index == 1 || index == 2) {
            let nextIndex = Int(index)
            let sourceIndex = Int(index)
            
            nextButton = glt_buttons[nextIndex]
            if ((sourceIndex - 1)>=0) {
                
               currentButton = glt_buttons[sourceIndex-1]
            }
...
//                if ((sourceIndex - 1)>=0) {
//                    let lastButton = glt_buttons[sourceIndex-1];
//                    lastButton.isSelected = false;
//                }
                currentButton.isSelected = false
                nextButton.isSelected = true
                // currentButton.setImage(UIImage.init(named: self.titles[sourceIndex]), for: .normal)
                //nextButton.setImage(UIImage.init(named: self.titlesS[nextIndex]), for: .normal)
           ...
           // sourceIndex = sourceIndex + 1
            ...
            if (index == 0 || index == 1 || index == 2) {
             ...
             if ((sourceIndex + 1) < self.glt_buttons.count) {
...
extension LTPageView {
    ...
    @discardableResult
    private func subButton(frame: CGRect, flag: Int, title: String?,titleS:String?, parentView: UIView) -> UIButton {
        let button = UIButton(type: .custom)
        button.frame = frame
        button.tag = flag
        button.setImage(UIImage.init(named: title!), for: .normal)
        button.setImage(UIImage.init(named: titleS!), for: .selected)
